...
.found_kernel_elf:
    ...
    push cx                 ; save sector count; will pop into ax later
    mov bx, [es:di + 26]    ; Starting cluster
    
    ; Load kernel.elf into memory at 0x2000:0x0000
    ; Need to read in chunks due to track boundary limitations
    mov ax, bx              ; AX = cluster number
    sub ax, 2               ; Subtract 2
    add ax, 33              ; Add first data sector LBA
...
.use_track_limit:
    ...
    ; Update ES:BX pointer (each sector is 512 bytes = 0x200)
    ; Move ES forward by (sectors * 32) paragraphs (512 bytes = 32 paragraphs)
    push dx
    mov dx, ax              ; DX = sectors read
    shl dx, 5               ; DX = sectors * 32 (paragraphs)
    mov ax, es
    add ax, dx              ; Add paragraphs to segment
    mov es, ax
    pop dx
    ; BX stays at 0
